...
async function redrawProviderUI(provider, data) {
    ...
    errorActionsHeader.style.userSelect = "none"; // Prevent text selection
    ...
    let errorCollapsed = localStorage.getItem(errorCollapseKey) !== 'false'; // Default to collapsed (true)
    ...
    errorChevron.style.fontSize = "14px"; // Smaller chevron
    errorChevron.style.marginRight = "6px"; // Space after chevron
    errorChevron.style.width = "1em"; // Consistent width
    errorChevron.style.textAlign = "center";
    errorActionsHeader.appendChild(errorChevron);

    // Use createTextNode for simple text content
    errorActionsHeader.appendChild(document.createTextNode("Error Code Actions"));

    // Content div that will be shown/hidden
    const errorContent = document.createElement("div");
    errorContent.id = `keyswitcher-error-content-${provider.secret_key}`; // ID for content
    errorContent.style.display = errorCollapsed ? "none" : "block"; // Set initial display state
    errorContent.style.marginTop = "5px"; // Space between header and table

    // Click handler for the header to toggle visibility
    errorActionsHeader.onclick = () => {
        errorCollapsed = !errorCollapsed; // Toggle state
        localStorage.setItem(errorCollapseKey, errorCollapsed ? 'true' : 'false'); // Save state
        errorContent.style.display = errorCollapsed ? "none" : "block"; // Update display
        errorChevron.textContent = errorCollapsed ? "▶" : "▼"; // Update chevron
        errorActionsHeader.title = `Click to ${errorCollapsed ? 'expand' : 'collapse'} Error Code Actions`; // Update title
    };

    // Append header and content placeholder to the section
    errorActionsSection.appendChild(errorActionsHeader);
    errorActionsSection.appendChild(errorContent);

    // --- Populate Error Actions Content (Table) ---
    // Load current preferences for this provider
    const errorPrefs = loadErrorCodePrefs(provider);
    // Get the defined error codes for this provider from the mapping
    ...
    if (!providerCodes || Object.keys(providerCodes).length === 0) {
        ...
        table.style.borderCollapse = "collapse"; // Nice table borders
        table.style.marginTop = "5px"; // Space above table

        // --- Table Header ---
        const thead = table.createTHead();
        const headerRow = thead.insertRow();
        // Header cell for the 'Action' column
        const thAction = document.createElement("th");
        thAction.textContent = "Action";
        thAction.style.textAlign = "left";
        thAction.style.padding = "4px 8px"; // Padding
        thAction.style.border = "1px solid #555";
        thAction.style.background = "#333a40"; // Header background
        headerRow.appendChild(thAction);

        // Create header cells for each error code, sorted numerically
        ...
        sortedCodes.forEach(code => {
            ...
            thCode.style.padding = "4px 8px";
            thCode.style.border = "1px solid #555";
            thCode.style.background = "#333a40";
            thCode.style.minWidth = "40px"; // Ensure columns have some width
            headerRow.appendChild(thCode);
        });

        // --- Table Body ---
        const tbody = table.createTBody();
        const actions = ['Rotate', 'Remove']; // The actions available

        actions.forEach(actionName => {
            const row = tbody.insertRow();
            // Cell for the action name (Rotate/Remove)
            const cellAction = row.insertCell();
            cellAction.textContent = actionName;
            cellAction.style.fontWeight = "bold";
            cellAction.style.padding = "4px 8px";
            cellAction.style.border = "1px solid #555";
            cellAction.style.textAlign = "left";

            // Create a cell with a checkbox for each error code
            sortedCodes.forEach(code => {
                const cell = row.insertCell();
                cell.style.textAlign = "center"; // Center the checkbox
                ...
                const actionValue = actionName.toLowerCase(); // 'rotate' or 'remove'
                checkbox.dataset.action = actionValue; // Store action in data attribute
                checkbox.dataset.code = code;         // Store code in data attribute
                checkbox.title = `${actionName} key automatically on error code ${code}\n(${providerCodes[code]})`;

                // Set initial checked state based on loaded preferences
                checkbox.checked = errorPrefs[code] === actionValue;

                // --- Checkbox Change Event Listener ---
                checkbox.addEventListener('change', async (event) => {
                    const changedCheckbox = event.target;
                    const currentAction = changedCheckbox.dataset.action;
                    const currentCode = changedCheckbox.dataset.code;
                    const isChecked = changedCheckbox.checked;

                    // Load the latest preferences before modifying
                    const currentPrefs = loadErrorCodePrefs(provider);

                    if (isChecked) {
                        // If this checkbox is checked, set its action for the code
                        currentPrefs[currentCode] = currentAction;

                        // Find and uncheck the *other* action's checkbox in the same column
                        const otherAction = currentAction === 'rotate' ? 'remove' : 'rotate';
                        const otherCheckboxSelector = `input[data-action='${otherAction}'][data-code='${currentCode}']`;
                        const otherCheckbox = table.querySelector(otherCheckboxSelector);
                        if (otherCheckbox && otherCheckbox.checked) {
                            otherCheckbox.checked = false; // Uncheck the sibling
                        }
                    } else {
                        // If this checkbox is unchecked, set the action to 'none' for this code
                        // This happens implicitly if the other checkbox is also unchecked,
                        // but explicitly setting it handles the case where this was the *only* one checked.
                        currentPrefs[currentCode] = 'none';
                    }

                    // Save the potentially modified preferences object
                    saveErrorCodePrefs(provider, currentPrefs);
                    // console.log(`KeySwitcher: Updated error prefs for ${provider.name} after change on ${currentCode}:`, currentPrefs); // Optional debug log
                });
        ...
        // Append the populated table to the content div
        errorContent.appendChild(table);

        // Add help text below the table
        const helpText = document.createElement("p");
        helpText.style.fontSize = "0.9em";
        helpText.style.color = "#aaa";
        helpText.style.marginTop = "10px"; // Space above help text
        helpText.style.lineHeight = "1.4"; // Improve readability
        helpText.innerHTML = `Select the desired automatic action (Rotate Key or Remove Key) for each HTTP status code reported by ${provider.name}. ` +
                             `Only one action can be active per code. If neither box is checked for a code, no automatic key action will occur for that error, ` +
                             `even if 'Toggle Auto Switching/Removal' is enabled globally. Changes are saved automatically when you check/uncheck a box. ` +
                             `Hover over error codes for descriptions.`;
        errorContent.appendChild(helpText);

    } // End else (if providerCodes exist)

    // Finally, append the entire error actions section to the main dynamic container
    dynamicContainer.appendChild(errorActionsSection);
    // --- END: Error Code Actions UI ---
